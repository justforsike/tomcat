 ...
 *	
 *	引导加载Catalina。这个应用程序构建了一个加载Catalina内部类（累加在catalina.home下的
 *	server文件夹下的所有的类）的类加载器，并且开始定期执行容器。这个迂回的方法是要保持
 *	Catalina的内部类（和其依赖的类，例如一个XML解析器）在系统类路径之外，并且是对应用级别
 *	的类透明的。
 *
...
public final class Bootstrap {
   ...
   /**
    ...
    //这个方法是从ExceptionUtils类中copy过来的，因为在启动的时候，那个类是不可见的
